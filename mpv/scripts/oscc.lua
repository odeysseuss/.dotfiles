...
local osc_styles = {
    ...
    timePosBar = "{\\blur2.4\\bord"
        .. user_opts.tooltipborder
        .. "\\1c&HEFEFEF\\3c&H000000\\fs"
        .. user_opts.tooltipfontsize
        .. "}",
}
            ...
            if not (element.slider.markerF == nil) then
                ...
                for _, marker in pairs(markers) do
                    if (marker > element.slider.min.value) and (marker < element.slider.max.value) then
                        ...
                        static_ass:rect_cw(s - 1.25, a + b, s + 1.25, b - a)
                    end
                end
            end
        end
                ...
                if mouse_hit(element) then
                    ...
                    if not thumbfast.disabled then
                        ...
                        if osd_w then
                            ...
                            local thumb_y = user_opts.layout == "bottombar"
                                    and ty / r_h - thumbfast.height - user_opts.tooltipfontsize / r_h - thumb_margin_y
                                or ty / r_h + thumb_margin_y
...
function bar_layout(direction)
    ...
    geo = { x = osc_geo.x + padd, y = line2, an = 4, w = btnw_left, h = btnw_left }
    ...
    if user_opts.volseekbar then
        ...
        geo = {
            x = osc_geo.x + osc_geo.w - volboxwidth - padd,
            y = geo.y,
            an = geo.an,
            w = volboxwidth,
            h = volboxheight,
        ...
        lo.geometry = { x = geo.x, y = geo.y, an = geo.an, w = geo.w, h = volhandlesize }
        lo.style = osc_styles.timecodesBar
        lo.slider.border = 0
        lo.slider.gap = 0
    ...
    lo.geometry = { x = osc_geo.x, y = line1, an = geo.an, w = osc_geo.w, h = heightbar }
    lo.layer = 15
    lo.style = osc_styles.timecodesBar
    lo.alpha[1] = user_opts.seekbaralpha
    if not (user_opts["seekbarstyle"] == "bar") then
    ...
    lo.geometry = { x = osc_geo.x, y = line1, an = geo.an, w = osc_geo.w, h = heighthandle }
...
function osc_init()
    ...
    ne.content = function()
        ...
        return ("\xee\x80\x86" .. osc_styles.smallButtonsLlabel .. " " .. aid .. "/" .. #tracks_osc.audio)
    end
    ...
    ne.content = function()
        ...
        return ("\xee\x80\x87" .. osc_styles.smallButtonsLlabel .. " " .. sid .. "/" .. #tracks_osc.sub)
    end
    ne.eventresponder["mbtn_left_up"] = function()
        set_track("sub", 1)
    end
    ...
    if user_opts.volseekbar then
        ...
        ne.slider.posF = function()
            return mp.get_property_osd("volume")
        end
    ...
    ne.content = function()
        ...
        return (min > 0 and string.format("  %sm%02.0fs", min, sec) or string.format(" %3.0fs", sec))
    end
    ...
    ne.eventresponder["shift+mbtn_left_down"] = function()
        show_message("Volume: " .. mp.get_property_osd("volume") .. "%")
    end
